...
object XBloomBleProtocol {
    ...
    fun frame(cmd: Int, seq: Int = LOAD_SEQ, payload: ByteArray): ByteArray {
        ...
        out.write(byteArrayOf(0x58, 0x01, 0x01, cmd.toByte(), seq.toByte(), 0, 0, 0, 0))
        out.write(payload)
    ...
    fun stageTemps(t1: Float = 110f, t2: Float = 90f): ByteArray {
        val p = ByteBuffer.allocate(9).order(ByteOrder.LITTLE_ENDIAN).put(1).putFloat(t1).putFloat(t2).array()
        return frame(0xA8, payload=p)
    }
    ...
    fun pours(recipe: BrewRecipe): ByteArray {
        ...
        recipe.pours.forEachIndexed { i, p ->
            var left = p.volumeMl
            while (left > 127) { segments.write(byteArrayOf(127, p.temperatureC.toByte(), p.pattern.code.toByte(), p.agitation.toByte())); left -= 127 }
            segments.write(byteArrayOf(left.toByte(), p.temperatureC.toByte(), p.pattern.code.toByte(), p.agitation.toByte(), ((256-p.pauseSeconds) and 0xff).toByte(), 0, (if(i==0) recipe.grinderRpm else 0).toByte(), p.flowRateTenthsMlPerSec.toByte()))
        }
